...
if(!ENDPOINT)throw new Error('S3_ENDPOINT is required (Cloudflare R2 endpoint)');
...
app.get('/health',async(_req,res)=>{try{await ensureBucket();res.json({ok:true,storage:'Cloudflare R2',s3Compatible:true,endpoint:ENDPOINT})}catch(e){res.status(503).json({ok:false,error:e.message})}});
...
app.listen(PORT,()=>console.log(`AitherFiles Cloudflare R2 S3 API listening on ${PORT}`));
