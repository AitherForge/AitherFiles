...
const ENDPOINT=process.env.S3_ENDPOINT||'https://stored.ge';
...
const s3=new S3Client({endpoint:ENDPOINT,region:REGION,forcePathStyle:true,credentials:{accessKeyId:process.env.S3_ACCESS_KEY,secretAccessKey:process.env.S3_SECRET_KEY}});
...
async function ensureBucket(){try{await s3.send(new HeadBucketCommand({Bucket:BUCKET}))}catch(e){if(e.$metadata?.httpStatusCode===404||e.name==='NotFound')await s3.send(new CreateBucketCommand({Bucket:BUCKET}));else throw e}}
app.get('/health',async(_req,res)=>{try{await ensureBucket();res.json({ok:true,storage:'stored.ge',s3Compatible:true})}catch(e){res.status(503).json({ok:false,error:e.message})}});
...
app.listen(PORT,()=>console.log(`AitherFiles S3 API listening on ${PORT}`));
